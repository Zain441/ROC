    ...
    public class vShooterMeleeInput : vMeleeCombatInput
    {
        ...
        protected virtual void UpdateArmsIK(bool isUsingLeftHand = false)
        {
            ...
            if (targetIK != null)
            {
                ...
                if (weapon && weapon.handIKTarget && Time.timeScale > 0 && !isReloading && !cc.actions && !cc.customAction && (!animator.IsInTransition(4) || isAiming) && !isEquipping && (cc.isGrounded || (isAiming || aimTimming > 0f)) && !cc.lockMovement && useIkConditions)
